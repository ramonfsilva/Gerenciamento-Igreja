...
<div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <h1 class="text-2xl font-bold text-text-primary">Entradas</h1>
        @can("income.create")
    ...
    <x-card :padding="false">
        <div class="p-4 flex flex-wrap items-center gap-3">
            ...
            <div class="ml-auto text-sm text-text-secondary">
                Total: <span class="font-semibold text-success">R$ {{ number_format($total, 2, ",", ".") }}</span>
            </div>
        </div>
    </x-card>
    ...
    @if($showForm)
        <x-card>
            <h2 class="text-lg font-semibold text-text-primary mb-4">{{ $editId ? "Editar Entrada" : "Nova Entrada" }}</h2>
            <form wire:submit="save" class="space-y-4 max-w-lg">
                ...
                <div>
                    ...
                    <select wire:model="member_id" class="w-full border-input focus:border-primary focus:ring-primary rounded-md shadow-sm">
                ...
                <x-card>
                    <div class="flex flex-col h-full">
                        <div class="flex items-start justify-between mb-2">
                            <div>
                                <p class="font-semibold text-text-primary">{{ $inc->description ?? $inc->category?->name }}</p>
                                <p class="text-sm text-text-secondary">{{ $inc->category?->name }}</p>
                            </div>
                            <span class="text-lg font-bold text-success shrink-0">R$ {{ number_format($inc->amount, 2, ",", ".") }}</span>
                        </div>
    ...
    @else
        <x-card>
            <p class="text-center text-text-secondary py-8">Nenhuma entrada no período.</p>
        </x-card>
    @endif
